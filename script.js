/* Reworked script.js to keep all original logic but render pixel-style bar charts.
   You can replace your old script.js with this file.
   NOTE: Chart.js is loaded from CDN in index.html as before.
*/
...
class FinanceManager {
    ...
    updateChart() {
        ...
        // Pixel-ish bar appearance: solid blocks with dark outline
        const datasets = [];
        if (chartView === 'income' || chartView === 'both') {
            datasets.push({
                label: 'Pemasukan',
                data: incomeData,
                backgroundColor: '#7fc8a9', // mint
                borderColor: '#4b8a6e',
                borderWidth: 3,
                barPercentage: 0.7,
        ...
        // Create bar chart (pixel blocky styling)
    ...
    /* --- the rest of the class is unchanged from your original logic --- */
    ...
    renderTransactions() {
        ...
        // Reverse order for latest first (do not mutate original array)
        const copy = [...this.transactions].reverse();
    ...
    updateBalance() {
        ...
        if (balance >= 0) {
            balanceElement.style.color = 'var(--muted)';
        ...
        // little bounce effect for pixel feel
